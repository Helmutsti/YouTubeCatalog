...
## Manutenzione (2026-07-25) — ripristino stato "scaricato" + riconciliazione job orfani

Due interventi nati da segnalazioni dell'utente, non milestone (correzioni + una riparazione dati).

**1. 83 video "scaricati" declassati per errore → ripristinati (riparazione dati).** L'utente aveva i video su `videosRoot: D:\YouTube\Video` (disco separato). Una **sync partita mentre quel disco non era raggiungibile** ha attivato l'auto-guarigione di `ingestPlaylistEntries` (`downloaded → none` quando il file non è su disco) su un'intera fonte — colpite soprattutto *Angelica ASMR* (93/95) e *ASMR Preferiti* (45). Rimettere i file a posto **non** li recuperava: l'auto-guarigione è **a senso unico**, non esiste l'inverso `none → downloaded` alla ricomparsa del file. Diagnosi sui dati reali: 83 video con `download: none` ma `video.localPath` valorizzato **e** file di nuovo presente su disco (83/83, 0 con dimensione incoerente col `sizeBytes` registrato). Riparazione una tantum (server fermato prima, per non far sovrascrivere la cache in memoria): flip `download → downloaded` **solo** su quei 83, tutto il resto dei metadati di download era intatto (la sync aveva cambiato solo il flag). `downloaded`: **237 → 320 (+83)**, verificato via API a server riavviato. I 23 `presence: removed` (0 con file) lasciati intatti: sono davvero rimossi da YouTube, altro asse. *La fragilità che ha causato tutto (disco offline trattato come "file cancellato") resta aperta come bug in `PIANO.md`.*

**2. Bug: job orfani "in caricamento all'infinito" (corretto alla radice).** Interrompendo un download e poi riavviando/uccidendo il processo, il job restava `running` per sempre nella UI: `jobManager.ensureLoaded()` caricava `jobs.json` così com'era, senza mai riconciliare i job lasciati `running`/`queued` da un processo morto. Siccome l'`AbortController` vive **solo in memoria** (`controllers`, mai in `jobs.json`), dopo un riavvio quel job era impossibile da chiudere: né interrompibile (`cancelJob` → "non supporta l'interruzione", nessun controller da recuperare) né cancellabile (`deleteJob` rifiuta `running`/`queued`). Caso reale: job `downloadSingle` `b94d1537…` (video `hCJZAGcV1ZE`) fermo `running` dal 23/07 — il video era comunque stato scaricato con successo il 24/07 (l'mp4 è su disco, integro), il job era solo un residuo. **Fix**: nuova `reconcileOrphanJobs()` in `core/src/jobs/jobManager.js`, chiamata da `ensureLoaded()` subito dopo aver popolato la Map (sia dal file consolidato sia dalla migrazione dal vecchio layout) — chiude come `failed` ogni job `running`/`queued` residuo ("Interrotto dal riavvio dell'applicazione (job orfano)"). Stesso principio della reconciliation di `catalogStore` (`downloading → none` all'avvio). Verificato: al riavvio 0 job `running`/`queued`, `b94d1537…` ora `failed` e cancellabile dalla UI. D'ora in poi ogni download interrotto + riavvio si auto-chiude.

---

## M64-M66 (2026-07-28) — installazione a tre comandi, pulizia del repo, e il core nativo misurato invece che ipotizzato

Tre milestone nate da un'unica sessione di domande dell'utente: *«ha senso riscrivere il core in C/C++/Rust? come si reinserisce nel flusso? conviene anche per la CLI? si può rendere l'installazione facile per chi non sa cosa fare, magari coi binari già in `tools/`? mi confermi che ffmpeg non va installato? e poi vorrei pulizia nella cartella e un README più semplice.»*

**Contesto della sessione, rilevante per capire le verifiche:** la copia di lavoro era un **clone nudo** — niente `node_modules`, niente `tools/`, niente `data/catalog.json`, niente `media/`. Invece di essere un ostacolo si è rivelata il banco di prova ideale: tutte le verifiche di M64 sono state fatte esattamente nella condizione dell'utente nuovo che installa da zero.

### M66 — Il core in Rust/C++: scartato, ma **coi numeri**, non per opinione

L'utente ha esplicitamente rifiutato di fermarsi all'analisi statica del codice («approfondisci: voglio un prototipo/benchmark reale»), quindi il punto è stato deciso misurando. Criterio fissato **prima** di misurare per non razionalizzare a posteriori: *se nessuna operazione supera ~100ms alla scala reale, si chiude*.

Benchmark su catalogo **sintetico** a forma dello schema vero (i 274 video reali sono su un'altra macchina — dichiarato), Node 24 / win32-x64, mediana su 5 giri con warm-up:

| | 274 video (reale) | 2.000 | 10.000 |
|---|---|---|---|
| `JSON.parse` | 1,4 ms | 10,1 ms | 51,5 ms |
| `JSON.stringify` | 1,8 ms | 12,5 ms | 61,2 ms |
| `searchVideos` "asmr sleep" (matcha) | 11,1 ms | 73,4 ms | 369,1 ms |
| `searchVideos` "sampuma grammar" (typo, 0 risultati) | 27,9 ms | 203,5 ms | 1.041,2 ms |
| sha256 di 343 MB (media reale/video) | 471 ms — **728 MB/s** | | |

**Verdetto**: nessun collo di bottiglia in JavaScript alla scala del progetto. Il core è 3.031 righe di cui il 24% (`ytdlpWrapper.js`) *aspetta* un processo esterno; il resto è I/O su JSON e orchestrazione di job. Lo sha256 è già OpenSSL in C via `createHash` e gira alla velocità del disco: Rust darebbe **zero**. Il costo reale del progetto è la logica di stato (flag ortogonali, ingest a due fasi, guarigione dei `downloaded`), che una riscrittura rende più costosa da cambiare, non più corretta. Si perderebbero l'invariante «progetto auto-portante» (oggi `git clone && npm install`; il Dockerfile QNAP vive proprio del fatto che non c'è nulla da compilare) e la decisione fondativa «un solo linguaggio».

**Conservato per il futuro**: l'architettura *reggerebbe* il cambio, perché `core/src/index.js` è già il contratto giusto. La strada sarebbe **napi-rs** (addon `.node`; CLI e server non cambierebbero una riga) e **non** un sidecar JSON-RPC, che romperebbe l'invariante «il CLI funziona senza server acceso». Scartata anche la CLI in Rust (`@inquirer/prompts` passa il tempo fermo ad aspettare un tasto); per il movente sospettato dietro la domanda — «un eseguibile da doppio-click» — la risposta giusta è **Node SEA/`pkg`**, non Rust, e l'utente ha scelto di rimandarlo.

**Errore commesso e corretto durante la misura** (vale la pena registrarlo): la prima stesura del benchmark re-importava `searchService.js` con un cache-buster sull'URL, ma `catalogStore.js` — importato *da* quel modulo, senza buster — resta lo stesso singleton e **cacha il catalogo in memoria**. Risultato: tutte e tre le scale cercavano sugli stessi 274 video, e i tempi non salivano (il sospetto è nato proprio da lì). Rifatto con **un processo separato per scala**. È lo stesso meccanismo già documentato in `documentazione.md` («il catalogo è tenuto in memoria da ogni processo»), ripresentatosi in un contesto nuovo.

**Scoperta collaterale, annotata in "Forse" e non risolta ora**: la ricerca fuzzy è O(N × lunghezza-testo × lunghezza-parola) e degrada male oltre il migliaio di video — e il **caso peggiore è il più comune** (una query con typo che non matcha forza la finestra scorrevole + Levenshtein su tutto il catalogo: 1,04s contro 369ms). Irrilevante oggi (274 video; 10.000 sarebbero ~3,4TB), e quando servirà il rimedio è **algoritmico in JS** (bail-out anticipato o indice n-gram, 100-1000×), non un cambio di linguaggio sullo stesso algoritmo quadratico (~10-30×).

### M64 — `npm run setup`: l'installazione passa da quattro decisioni a un comando

**Il problema, misurato sul clone nudo:** installare richiedeva di sapere *cosa* scaricare, *con che nome* (diverso per OS), *dove* (`tools/`, cartella che **non esiste** in un clone fresco) e di procurarsi **due** binari ffmpeg. Quattro decisioni; sbagliandone una l'errore arrivava molto dopo, come `spawn ENOENT` a metà download.

**Decisione presa con l'utente** — fra "script che scarica" / "binari committati in `tools/`" (la sua proposta iniziale) / "script + release .zip": **script che scarica**. Controproposta argomentata e accettata perché i binari committati costerebbero ~100-190MB *per aggiornamento* nella history di git (che non li deltifica) e, soprattutto, **un yt-dlp pinnato smette di scaricare** quando YouTube cambia — l'utente si ritroverebbe download falliti senza capire perché. Lo stesso ragionamento che il `docker-compose.yml` già applicava montando `tools/` come volume. Menzionate anche le licenze (le build ffmpeg statiche comuni sono GPLv3).

**Realizzato:**

- **`scripts/setup.mjs`** (`npm run setup`), **zero dipendenze npm nuove**: `fetch` nativo + `readZip`, il lettore ZIP già scritto a mano nel progetto per i backup (`core/src/lib/zip.js`) — riuso, non una libreria in più, coerente con l'invariante del core senza dipendenze. Avanzamento a passi del 5% (non un `\r` per chunk: su output non-TTY produrrebbe migliaia di righe). Scrittura **atomica** tmp+rename come `catalogStore`, così un'interruzione non lascia mai in `tools/` un binario troncato (che fallirebbe in modo molto più confuso di uno assente); `chmod +x` su non-Windows.
- **Fonte unica dei nomi dei binari**: nuova `expectedToolNames(platform)` in `core/src/config.js`, usata da `getPaths()`, dal setup e dal preflight. Prima la convenzione viveva solo dentro `getPaths()` e lo script l'avrebbe duplicata a mano — due copie che possono divergere in silenzio. Dettaglio catturato qui: su Linux ARM l'asset ufficiale si chiama `yt-dlp_linux_aarch64` ma va **salvato** come `yt-dlp_linux`, che è ciò che il codice cerca a runtime.
- **Rilevamento di ffmpeg già nel PATH**: se c'è, il setup **non** scarica ~160MB inutili (`getPaths()` ricade già sul PATH). Ramo esercitato davvero su questa macchina.
- **Preflight nel core** (`core/src/preflight.js`, `checkTools()`/`reportToolsOnStartup()`): controlla yt-dlp, ffmpeg (config → `tools/` → PATH, verificato con uno `spawnSync` vero, non un `existsSync`) e **ffprobe accanto a ffmpeg** — che yt-dlp usa per ispezionare i flussi e che è facile dimenticare. Chiamato all'avvio di **server** e **CLI**; **non blocca** l'avvio (sfogliare il catalogo e riprodurre i già scaricati funziona comunque). Vive nel core, non negli adapter, come da principio del progetto.
- **Dettaglio del CLI**: il preflight non può usare `console.warn`, perché il primo `clearScreen()` di `mainMenu()` lo cancellerebbe. Passa da `setMessage()`, che fa sopravvivere il messaggio esattamente a una schermata — la meccanica già introdotta per il reset dello schermo.

**Verifica end-to-end reale (B3), tutta sul clone nudo:**
1. *Prova negativa* con `tools/` assente → messaggio corretto con rimando a `npm run setup`, invece di `spawn ENOENT`.
2. `node scripts/setup.mjs` → yt-dlp 17,4MB scaricato, **yt-dlp 2026.07.04** verificato eseguibile; ffmpeg saltato perché nel PATH.
3. `--force` → scaricato lo zip BtbN da **160,9MB** ed estratti `ffmpeg.exe` (137,9MB) e `ffprobe.exe` (137,7MB) **con il `readZip` del progetto** — la parte più a rischio, verificata su un archivio reale di quella dimensione.
4. **Download reale end-to-end** in cartella temporanea (per non toccare i dati): yt-dlp ha scaricato due flussi separati (video 242 + audio 251), `tools/ffmpeg.exe` li ha fusi in mp4, e `tools/ffprobe.exe` conferma `vp9,320,240` + `opus`. **L'intera toolchain funziona partendo da un clone nudo con un solo comando.**
5. `npm install` + avvio reale del server → `GET /api/videos` risponde `[]`; riavvio con `tools/` nascosta → l'avviso compare come previsto.
6. Pulizia (B4): rimossi il download di prova e i file `data/*.json` creati dai test; `data/config.json` era stato auto-creato come effetto collaterale di `getPaths()` durante il benchmark ed è stato rimosso. `git status` riportato a pulito, `package-lock.json` (modificato solo da rumore di npm: rimozione di marcatori `"peer": true`) riportato indietro per non sporcare il diff.

**Confermato all'utente**: **ffmpeg non va installato** — bastano gli eseguibili. `getPaths()` passa la cartella `tools/` a yt-dlp via `--ffmpeg-location`; niente PATH, niente installer. Con tre precisazioni: servono **due** file (`ffmpeg` *e* `ffprobe`), devono essere build **statiche**, e su Linux/macOS serve `chmod +x` (che ora fa il setup).

### M65 — Pulizia e README essenziale

- **`.env.example` in radice eliminato**: dichiarava `NODE_ENV`/`PORT` ma era **codice morto** — `grep -rn "process.env" core packages` non trova nulla, la porta viene da `data/config.json`. (`packages/web/.env.example` è reale — `VITE_API_BASE_URL`, M47 — e resta.)
- **`.gitignore`**: i sette nomi di binario elencati a mano sostituiti da `/tools/` intera. Copre anche i `.tmp` della scrittura atomica e i nomi di piattaforme future senza dover aggiornare il file.
- **`data/config.example.json` riallineato al default reale** di `config.js`: mancava `videosRoot` e aveva `vlcPath` su `Program Files` mentre il default nel codice è `Program Files (x86)`. Un esempio che non combacia col default è peggio di nessun esempio.
- **README da 228 a 95 righe**: cos'è, tre comandi, uso, configurazione, cookie, backup, e una tabella di rimandi. Il materiale avanzato **non è stato buttato** ma spostato in **`docs/avvio-avanzato.md`** (112 righe): modello a server unico, sviluppo a due processi con hot-reload, le due modalità LAN, e la digressione — preziosa e verificata sul campo — su *usa l'IP e non il nome del PC* (NetBIOS) con il ripiego sul firewall Windows.
- Aggiornato l'albero della struttura in `progetto.md` (unico documento condiviso che descrive i file): rimosso `.env.example`, aggiunti `scripts/setup.mjs`, `core/src/preflight.js`, `docs/avvio-avanzato.md`, `docs/DOCKER.md`, e la nota che `tools/` non è versionata.

**Limite noto, non risolto:** `npm run setup` scarica i binari per la piattaforma **dell'host su cui gira**. Chi prepara da Windows il volume `tools/` di un container Linux (scenario QNAP di `docs/DOCKER.md`) otterrebbe `yt-dlp.exe`, non `yt-dlp_linux`: per quel caso `DOCKER.md` resta la guida, con il download manuale. Non affrontato perché avrebbe richiesto un flag di piattaforma di cui non è chiaro il bisogno reale.

## M74 — Ritorno a Node: il ramo `rust-core` chiuso, l'albero JS ripristinato

**Decisione dell'utente**, dopo aver visto girare la riscrittura in Rust: *non gli
è piaciuta*, e vuole **la massima compatibilità** — un solo linguaggio, `git clone
&& npm install` che basta ovunque, nessuna build precompilata per tripla. Le
...
Node seguendo il disegno di `rust-core`?". Il confronto fra i due core, fatto
leggendo il codice e non a memoria, dice che **`main` è un superset**: il ramo
Rust non aveva fonti/playlist + sync, storico dei job su disco, annullamento del
singolo job, backup/restore zip, reorganize, avatar dei canali, analisi dei
formati e `audioStrategy` — 12 endpoint e 4 concetti interi, elencati uno per uno
in `DIFFERENZE.md` su quel branch. Il costo reale della riscrittura non era stato
scrivere Rust: era **ricostruire anni di decisioni**, e in gran parte non era
stato pagato. Ripartire da `main` significa che per la React app *non manca
niente*, perché è il core contro cui è stata scritta.

**Fatto:** branch `node-core` da `rust-core`; albero di `main` ripristinato
(`core/`, `packages/*`, `scripts/`, `docs/`, Dockerfile, manifest); rimossi
`ondo-core/`, `cli/`, `ondo-web/`, `Cargo.toml`, `Cargo.lock`, `ARCHITETTURA.md`,
`target/`. Nessuna libreria utente da convertire: `ondo-data/` era vuota, quindi
nessuna migrazione `library.json` → `catalog.json`.

**Ordine dei lavori deciso con l'utente**: prima core e CLI, il web dopo —
l'albero della web app è comunque già in posizione verbatim da `main`, quindi
"riportarla 1 a 1" sarà verifica, non riscrittura.

## M75-M80 — Ciò che il ramo Rust aveva scoperto, portato nel core JS

Cinque cose che valevano **indipendentemente dal linguaggio**. Verificate tutte
insieme con due download veri in parallelo (in fondo).

### M75 — Presa in carico atomica dei video (`claimVideosForDownload`)

**Il difetto, trovato leggendo il codice prima di toccarlo:** `readCatalog()`
restituisce l'oggetto **vivo** in memoria, e `downloadPending` lo leggeva,
filtrava i candidati, e poi marcava `downloading` un video per volta attraverso
molti `await`. Con la coda a un worker era innocuo; con N download in parallelo il
filtro decide su dati già superati e **lo stesso video si scarica due volte**. Il
pool non introduceva questo bug: lo *rivelava*. Per questo M75 è stata chiusa
**prima** di M76, non dopo.

`claimVideosForDownload(ids, { allowRedownload })` verifica-e-marca in un'unica
mutazione dentro il mutex di `updateCatalog`, e restituisce la fotografia
(id/titolo/url) dei soli video presi in carico, catturata là dentro.

**Due scelte cambiate strada facendo, contro il primo disegno:**

- **Niente copia profonda in `readCatalog()`**, che era la mossa ovvia. Non
  risolve niente: una decisione presa su una copia è *stabilmente* vecchia invece
  che *casualmente* vecchia, e resta sbagliata allo stesso modo. In più
  costerebbe a ogni richiesta dell'API (1,4ms di solo `JSON.parse` sul catalogo
  reale, misurati in M66) per comprare una falsa sicurezza. Resta l'oggetto vivo,
  con un commento che dice cosa non farci.
- **Presa in carico dentro il ciclo, non in blocco.** Marcare tutti i 20 video del
  lotto in partenza mostrava venti video "in corso" mentre ne scaricava uno, e
  all'interruzione i non partiti restavano appesi in quello stato — rompendo
  l'invariante già dichiarata a `signal.aborted` («i successivi restano da
  scaricare, **mai avviati**, non solo saltati»).

### M76 — Il pool: `parallel` job insieme, e perché non sono thread

`jobManager` aveva un booleano `processing`. Adesso ha un contatore, e **non ci
sono thread**: il lavoro pesante di un download (rete, decodifica, mux) sta dentro
yt-dlp e ffmpeg, che sono processi separati schedulati dal sistema su tutti i
core; il nostro codice fa spawn, legge stdout riga per riga e parsa la
percentuale, cioè I/O. I thread del pool Rust passavano la vita **bloccati su
`read()` di una pipe** — N `spawn` concorrenti danno lo stesso parallelismo reale.

- `config.jobs.parallel`, **default 1** = comportamento storico: alzarlo è una
  scelta dell'utente, non un cambio che gli arriva addosso con un aggiornamento.
- **Modificabile a caldo**: il tetto si rilegge dalla config a ogni giro invece di
  essere memorizzato, quindi cambiarlo dalle impostazioni ha effetto subito senza
  un canale dedicato. Alzarlo fa partire altri job; abbassarlo **non interrompe
  niente** — quelli in corso finiscono, non vengono rimpiazzati.
- **Un solo evento terminale per job**, garantito nel `finally`. È la regola su
  cui si appoggia tutto ciò che ascolta, e sul branch Rust era stata rotta una
  volta (un `done` non emesso) lasciando appeso per sempre chi aspettava.
- `cancelJob(id)` continua a valere **per singolo job** (AbortController →
  `spawn({signal})` → uccide *quel* yt-dlp). È una cosa che il pool Rust non
  aveva: annullava tutto o niente.
- `persistStore()` resta senza mutex e **continua a non servirne uno**: ogni
  mutazione della Map è sincrona e la scrittura è sincrona, quindi fra la modifica
  in memoria e il salvataggio non c'è nessun `await` in cui infilarsi. Il pericolo
  del parallelismo in Node non è la race dentro un turno — non esiste — ma
  l'interleaving *fra* due `await`.

**Deliberatamente NON fatto: `worker_threads`.** I soli candidati CPU-bound in JS
sono lo zip del backup su ~94GB (il più probabile), lo sha256 di un file grande e
la ricerca fuzzy a grande scala. Regola presa: **nessun worker senza una misura
prima** — un worker aggiunge un confine di serializzazione, e metterlo "per
sicurezza" ripeterebbe in piccolo l'errore del §9 di `rust-core.md`.

### M77 — La CLI non è più bloccante

Con il pool i download **continuano mentre un menu è aperto** (l'event loop gira
anche quando `@inquirer` aspetta un tasto), quindi la frase di `progetto.md` «non
esiste uno stato in cui si può navigare al menu mentre un job è in corso» ha
smesso di essere vera per costruzione. Aggiunti: la scelta «resta a guardare» /
«torna al menu» dopo l'avvio (con "resta a guardare" come **prima voce**, cioè il
default premendo Invio: chi non vuole cambiare abitudini non deve imparare
niente), la voce di menu **«Download in corso (N)»** che compare solo se c'è
qualcosa in movimento, l'interruzione del singolo download da lì, e la conferma
all'uscita se ci sono job attivi (uscire li uccide, e non è più ovvio come quando
il menu era bloccante).

**Niente raw mode, niente console dal vivo.** La console «Download rapido» del
ramo Rust esisteva perché là serviva un ciclo di ridisegno proprio; qui basta
rileggere la fotografia da `listJobs()`/`getPoolStatus()` quando si disegna un
menu — l'interfaccia non ricorda niente, perché un evento si consuma una volta
sola mentre una fotografia si può richiedere sempre.

### M78 — La risoluzione si misura sul file (`core/src/ytdlp/probe.js`)

I metadati di yt-dlp (`-J`) descrivono il **miglior formato disponibile**, non
quello scaricato: con il tetto di risoluzione (M56) il catalogo registrava
`3840×2160` per un file che su disco è `1280×720`. Non un'approssimazione: un dato
falso. `ffprobe` era già in `tools/` e serviva solo a farsi controllare la
presenza. `probeResolution()` misura il file finale (dopo la fusione) e **non
lancia mai**: se ffprobe manca o non risponde si tengono i numeri dei metadati,
perché far fallire un download da 600MB già completato per una misura sarebbe
peggio del dato approssimativo.

### M79 — `--js-runtimes` additivo

Era inchiodato a `node`. yt-dlp ne supporta quattro (`deno`, `node`, `quickjs`,
`bun`), abilita di default solo `deno`, e **il flag è additivo**: elencarli tutti
significa "usa il migliore che trovi". Inchiodarlo a uno trasformava una dipendenza
*di yt-dlp* in una dipendenza *inventata da noi* — su una macchina con Deno e
senza Node i download morivano con un 403 senza motivo apparente. Che *questo*
progetto giri su Node non c'entra: il runtime serve a yt-dlp, che è un processo a
sé.

### M80 — Un lock su `data/`, invece di una regola da ricordare

Server e CLI aperti insieme si sovrascrivevano il catalogo a vicenda. Era
documentato come regola («quel processo va riavviato»), cioè il danno dipendeva
dalla memoria dell'utente. Adesso `data/.lock` (creazione atomica con flag `wx`,
pid + battito, rilascio su `exit`/SIGINT/SIGTERM) fa fallire il secondo processo
con un messaggio chiaro.

**Un difetto vero, trovato dalla verifica e non dalla lettura.** La prima versione
considerava abbandonato anche un lock **col battito fermo ma il processo vivo**, e
la prova fra due processi ha mostrato che quel lock si lasciava rubare. È il
difetto peggiore possibile qui, perché il caso in cui capita è concreto: il
proprietario che non batte è tipicamente uno con l'event loop occupato da qualcosa
di lungo e sincrono — **lo zip del backup su 94GB** — cioè *proprio mentre sta
scrivendo*. Regola corretta: abbandonato **solo se il processo non esiste più**. Il
battito non decide più, racconta soltanto: se il proprietario è vivo ma zitto da un
pezzo, il messaggio lo dice e suggerisce di cancellare il file a mano. Il prezzo è
che un pid riciclato tiene il lock finché non lo si cancella — un guaio
recuperabile, mentre un catalogo sovrascritto non lo è.

### La verifica (B3, end-to-end reale)

Banco automatico su 18 controlli (pool, evento terminale unico, tetto a caldo,
presa in carico concorrente, 25 mutazioni concorrenti senza perdite, ffprobe su
file generato con ffmpeg) + 7 controlli del lock **fra processi veri** + server
avviato e interrogato sugli endpoint + CLI avviata.

E soprattutto **due download veri in parallelo con tetto a 360p**:

| video | risoluzione registrata | cosa avrebbero detto i metadati |
|---|---|---|
| `jNQXAC9IVRw` "Me at the zoo" | **320×240 @ 15fps** | 240p (unico formato) |
| `aqz-KE-bpKQ` "Big Buck Bunny 60fps 4K" | **640×360 @ 30fps** | 4K @ 60fps |

Entrambi chiusi in 14s insieme, un solo evento terminale ciascuno, file nel layout
canonico `<Creator>/<Titolo> [<id>].mp4`. La seconda riga è M78 vista funzionare
sul dato vero: senza ffprobe il catalogo avrebbe scritto 3840×2160 @ 60fps di un
file che è 640×360 @ 30. Pulizia a fine prova (B4): `media/` e i file di `data/`
rimossi, cartella riportata allo stato di partenza.

**Limite noto della prova, non risolto:** `deleteVideoCompletely` pretende che il
video sia prima archiviato, quindi la pulizia del catalogo è passata dalla
rimozione diretta dei file di `data/` invece che dall'API. Non è un difetto del
core (la regola è voluta), è una cosa da sapere quando si scrive un banco di prova.

## M86 — La CLI torna identica a quella del ramo Rust (traduzione 1:1)

**Il difetto che ha innescato la milestone.** M74 aveva ripristinato l'albero di
`main` con un ragionamento corretto **sul core** (quello di `main` è un superset
di quello Rust: fonti, sync, storico job, backup, reorganize, avatar…), ma
insieme al core è tornata anche la **CLI di `main`**, che è quella dell'era M6:
dodici voci costruite attorno a `fonti → sincronizza → rivedi novità`. La CLI del
ramo Rust era **un'altra applicazione**: quattro voci, la libreria come unico
centro, nessuna nozione di playlist. Constatazione dell'utente, alla lettera:
«non somiglia PER NIENTE a quella rust». Aveva ragione, e non su dei dettagli.

**Le tre decisioni prese con l'utente prima di scrivere codice.** (1) *Menu
identico e basta*: le otto voci che il ramo Rust non aveva **escono dalla CLI**
(restano nel core e nella web app — è la CLI che torna com'era, non il progetto
che perde funzioni). (2) *Traduzione fedele*, con le differenze residue **elencate
a lavoro finito** invece di decise in autonomia. (3) *Download rapido in raw
mode*, come l'originale.

### Come è stata fatta: un adattatore, per poter confrontare i sorgenti

Il problema pratico della traduzione non era la logica ma la **forma**: i moduli
Rust chiamavano `Library` e `Downloader`, oggetti con metodi **sincroni** su uno
stato in memoria; questo core è a funzioni **asincrone** su `data/catalog.json`.
Spargere `await` dentro i menu avrebbe reso i sei file irriconoscibili rispetto
all'originale, e quindi impossibili da confrontare quando qualcosa diverge.

Da qui `packages/cli/ondo.js`: `Library` tiene una **fotografia** del catalogo,
ripresa da `refresh()` a ogni giro di menu (che è esattamente ciò che faceva la
`Library` di Rust, che il catalogo lo teneva in memoria), così `lib.list(filter)`
resta sincrona e i sei moduli restano riga per riga confrontabili. Le uniche righe
asincrone sono le mutazioni. `Downloader` è il pool di job del core visto come il
pool di sentinel: `pushWith` accoda, `statuses()` è la fotografia dei job della
sessione — come là, **non c'è stato da perdere** uscendo da una schermata.

File nuovi, uno per modulo Rust: `ui.js` ← `ui.rs`, `library.js` ← `library.rs`,
`search.js` ← `search.rs`, `settings.js` ← `settings.rs`, `quick.js` ← `quick.rs`,
`cli.js` ← `main.rs` (riscritto da zero). Rimossi `quickDownload.js` (la
ricostruzione con `readline` di M85, superata da `quick.js` in raw mode) e
`vlcQueuePlayer.js` (non esisteva nel ramo Rust).

### Colmature nel core (dove doveva stare la logica, non nella CLI)

- **`services/playbackService.js`** — `playVideo(id, {mode})` con VLC: era
  `Library::play`, quindi core, non CLI. Stessi messaggi d'errore dell'originale.
- **`services/libraryViewService.js`** — la parte di lettura di `lib.rs` e
  `search.rs`: `FILTER`/`STATE` con le loro etichette, `listByFilter`,
  `countByFilter`, `authors`, `byAuthor`, `missingFiles`, `retryVideo`, e
  `libraryVideo()`, **l'unico punto di traduzione** fra i flag ortogonali di
  questo core (M25) e il `Video` di `model.rs`. La corrispondenza dei filtri vive
  qui e in nessun altro posto: `All` = non nascosti, `Downloaded` = scaricati e
  non nascosti, `Pending` = `none|downloading`, `Archived` = `hidden`, `Removed` =
  `presence: removed`.
- **`quality.js`** — la `Quality` di `config.rs` (`"best" | "ask" | {height:N}`)
  in `data/config.json`. «Chiedi ogni volta» **non è un tetto**: è l'assenza di
  una decisione presa in anticipo, e il tetto vero viaggia col singolo download.
- **Ricerca esatta** — il punteggio di `search.rs`: AND su tutte le parole,
  sottostringa esatta, pesi 8/5/3/1, +8 se il titolo comincia con la query.
  **Nessuna tolleranza ai typo**: nel ramo Rust era stata togliata di proposito
  perché su descrizioni lunghe restituiva risultati quasi casuali. Convive con la
  `searchVideos` fuzzy, che resta quella della web app: sono due ricerche diverse.
- **`removeVideoFromLibrary(id, {deleteFiles})`** — `Library::remove`, e **non**
  `deleteVideoCompletely`: nessun gate "prima archivialo" (la CLI chiede conferma
  due volte, la seconda solo per i file) e i file si cancellano su richiesta.
- **`findJsRuntime()`/`JS_RUNTIME_NAMES`** in `preflight.js` — `config::js_runtime`:
  serve al controllo all'avvio e al pannello delle impostazioni.

### Due aggiunte al job manager, per motivi non decorativi

- **`ctx.note({...})` + `onJobNote` + campo `note` sul job.** Una riga di
  avanzamento ha bisogno del **titolo appena la risoluzione l'ha reso noto**, non
  a download finito (`summary`): è l'equivalente di `Event::Resolved`/`Event::Phase`
  del sentinel. L'alternativa era parsare le righe di log — cioè legare la CLI
  alle parole esatte di un messaggio scritto per gli umani. La nota resta **anche
  sul record**, quindi chi arriva su una schermata a job già avviato la rilegge
  invece di aver perso l'unico evento che la portava.
- **`nudgeJobPool()`.** Il tetto di parallelismo si rilegge a ogni `pump()`, ma
  `pump()` lo chiamano solo l'arrivo di un job e la fine di un altro: **alzare**
  `jobs.parallel` dalle impostazioni non avrebbe fatto partire niente finché non
  succedeva una delle due cose — e con la coda piena e nessun job in corso non
  sarebbe successo mai. Difetto trovato scrivendo la voce "Download in parallelo",
  non a posteriori.

### La verifica (B3, end-to-end reale)

1. **Percorso dati sul catalogo vero, in sola lettura**: fotografia, sette
   conteggi, righe d'elenco, autori, ricerca (compresa la prova che un typo dà 0
   risultati, come l'originale), pannello percorsi, pool, durate/dimensioni.
2. **La CLI vera pilotata a tasti** (frecce e Invio su stdin, output catturato):
   menu principale → Libreria → conteggi → «tutti i video» → un video (pannello
   statistiche + azioni contestuali: `Riproduci`/`Solo audio` su uno scaricato,
   nessuno `Scarica ora`) → indietro a ogni livello → Impostazioni → Stato e
   percorsi (● / ○ su radice, video, copertine, metadati, i tre binari, il
   runtime JS trovato, VLC) → Qualità predefinita (scelta **scritta** in
   `config.json`, `✓` di conferma, etichetta del menu aggiornata) → uscita 0.
3. **Download rapido in raw mode**: link digitato → selettore di risoluzione
   disegnato dentro la schermata («Che risoluzione per …?», ↑↓ che muovono il `❯`,
   «Invio per far partire il download») → Esc che **rimette il link nel prompt** →
   Esc che torna al menu → nessun download avviato. Provato sia isolato sia
   arrivando dai menu, che è il caso in cui una schermata così si mangia i tasti
   di quella dopo: non succede.
4. **L'originale Rust compilato e avviato** (`cargo build -p ondo-cli`) per il
   confronto: intestazione e messaggio di preflight combaciano alla lettera
   (`── ondo · N video in <radice>`, `✗ non trovo: …. I download falliranno.`).
   **Limite della prova:** `dialoguer` rifiuta uno stdin non-TTY («not a
   terminal»), quindi l'originale **non è pilotabile a tasti** come la versione
   Node — il confronto A/B delle schermate interattive va fatto in un terminale
   vero, dall'utente. Dichiarato invece che simulato.

**Pulizia (A7/B4)**: `quality` rimossa da `config.json` (com'era prima delle
prove), 2 video prima / 2 dopo, coda link vuota, nessun flag `favorite`/`hidden`
toccato, nessun lock residuo, cartella di build Rust e worktree temporaneo
rimossi.

### Differenze residue, e perché restano

Non tutto si può tradurre: `dialoguer` e `@inquirer` non sono la stessa libreria.

- **Esc/Ctrl-C** in un menu: in `dialoguer` `interact_opt` torna "niente scelto";
  con `@inquirer` non esiste, quindi Ctrl-C viene **mappato** su quel significato
  (torna indietro, ed esce dal menu principale). Esc dentro un elenco non annulla:
  si torna con «← indietro», che nell'originale c'è comunque in ogni menu.
- **I campi di testo** (`Cerca`, i percorsi) partono col valore attuale come
  *default* (Invio lo conferma) invece di averlo **già scritto e modificabile**
  come `with_initial_text`.
- **Il pannello statistiche è allineato in colonna**, l'originale no: in Rust
  `{:<12}` su un testo colorato non pativa la larghezza (l'impaginazione conta
  anche i codici di colore), quindi l'allineamento che l'autore aveva scritto non
  si vedeva. Qui si vede — scelta consapevole di seguire l'intenzione.
- **«Cartella metadati» è diventata «Cartella copertine»**: in questo core i
  metadati grezzi sono **un file** (`data/metadata.json`), non una cartella
  riposizionabile. Il pannello mostra comunque entrambe le righe.
- **`Filter::Pending`** significa qui "tutto ciò che non è ancora sul disco", e
  con le fonti/playlist attive comprende **anche i video mai scaricati arrivati da
  una sync** — nel ramo Rust entravano in libreria solo i link decisi a mano.
- **Il preflight mostra sempre un percorso assoluto**; l'originale, quando non
  riusciva a risolvere un binario, mostrava il nome nudo (`yt-dlp.exe`).
- **La CLI non applica più `Library::apply`**: là il pool riferiva e la libreria
  scriveva; qui è il **job** che scrive il catalogo, quindi `pump()` si limita a
  riprendere la fotografia. Stesso effetto, un passaggio in meno.

### Difetti trovati strada facendo

- **`@inquirer` stampava un `?` e una riga di aiuto** («↑↓ navigate • ⏎ select»)
  che `dialoguer` non ha. Tolti con un tema che azzera prefisso e `keysHelpTip`:
  con prefisso e messaggio vuoti il prompt scarta la prima riga per intero. Senza
  questo la schermata *somigliava* all'originale ma non lo era.
- **Un falso allarme diagnostico da annotare**: il primo test pilotato sembrava
  dire che l'Invio non arrivava alla schermata quick. Era il driver di prova a
