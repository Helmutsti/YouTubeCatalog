...
**Fra processi Ondo la regola è applicata dal codice** (M80): server e CLI prendono un lock consultivo `data/.lock` all'avvio (`acquireDataLock`), e il secondo che parte fallisce con un messaggio invece di sovrascrivere il catalogo del primo. Il lock si considera abbandonato **solo se il processo che lo teneva non esiste più** — non basta che abbia smesso di battere, perché un proprietario vivo ma zitto è tipicamente uno con l'event loop occupato da un'operazione lunga e sincrona (lo zip del backup), cioè proprio uno a cui non va portata via la libreria. Restano scoperti gli script esterni, che il lock non lo prendono.

**Dentro un processo, con più job in parallelo** (M76): l'unico punto che muta il catalogo è `updateCatalog()` (mutex asincrono + scrittura atomica). Chi deve *decidere* sulla base di uno stato e poi scriverlo non può leggere prima e mutare dopo — fra i due `await` un altro job può aver cambiato tutto: si usa `claimVideosForDownload()`, che verifica e marca in un'unica mutazione. `readCatalog()` restituisce l'oggetto **vivo**, non una copia: va letto dentro un turno, mai tenuto da parte attraverso degli `await`.
...
YouTubeCatalog/
  ...
  core/                           # LE MINI API: libreria di funzioni, richiamabile da CLI e WebGUI
    ...
    src/
      ...
      lock.js                       # acquireDataLock() (M80): lock consultivo data/.lock, un processo per libreria
      ...
      ytdlp/probe.js                  # probeResolution() (M78): risoluzione/fps MISURATI sul file con ffprobe
      ...
      jobs/jobManager.js              # pool a N job (config.jobs.parallel, M76) + EventEmitter, persistenza storico
...
Non esiste una scorciatoia "play per id" a comando digitato: la navigazione **Guarda** (canale → video → modalità) è l'unico modo per riprodurre un video partendo da zero, coerente col vincolo "niente comandi scritti a mano" — **Cerca** e **Scarica video singolo** sono le eccezioni minime e deliberate a questo vincolo, gli unici punti oltre "Aggiungi fonte" dove si digita testo libero, perché una ricerca o un link incollato non hanno senso senza testo digitato.

**I download non bloccano più il menu** (M77). Prima il design era bloccante — un solo flusso interattivo alla volta, quindi non esisteva uno stato in cui si potesse navigare mentre un job girava. Con il pool (M76) i download continuano **anche mentre un menu è aperto**: l'event loop gira anche quando `@inquirer` aspetta un tasto, e il lavoro vero è comunque dentro yt-dlp, che è un processo a sé. Di conseguenza: dopo aver avviato un download si sceglie fra **«resta a guardare i log»** (prima voce, cioè il default premendo Invio: è il comportamento di sempre) e **«torna al menu»**; una voce **«Download in corso (N)»** compare in cima al menu principale solo se c'è qualcosa in movimento, e da lì si può guardare o **interrompere un singolo** download; uscire con job attivi chiede conferma, perché uscendo muoiono. Niente raw mode e niente console a schermo pieno: lo stato non vive nella CLI, che lo rilegge da `listJobs()`/`getPoolStatus()` ogni volta che disegna.
