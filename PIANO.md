...
6. **"Riorganizza libreria": cosa significa "da spostare"?** — segnalato dall'utente come poco chiaro nella UI (`LibraryPage`/`reorganizeLibrary`): il numero mostrato ("N da spostare") non è immediatamente comprensibile senza già conoscere il funzionamento interno (file non ancora nel layout canonico per-creator). Da rivedere il testo/spiegazione mostrata, o aggiungere un dettaglio di cosa significa concretamente prima di eseguire.
7. **"Scarica video" non mostra mai lo storico, solo durante un download** — oggi `SingleDownloadPage` non ha nessuna vista sullo storico dei download singoli già fatti; per vederlo bisogna passare dalla pagina Job, e solo un download appena avviato ci finisce automaticamente (redirect). Richiesto: poter vedere sempre lo storico (non solo mentre si scarica) e poter **cancellare i job** dallo storico, sia singolarmente sia in blocco — funzionalità di cancellazione oggi assente del tutto (né singola né massiva, da nessuna pagina).
8. **Provider `channel` per il monitoraggio di interi canali** (ex M13, spostata qui perché non abbastanza definita per essere una milestone) — l'idea originale del progetto era lasciare un punto di estensione pulito in `sourceProviders` per aggiungere in futuro il monitoraggio di un intero canale YouTube (non solo playlist). Verificato però che oggi `sourceProviders/playlistProvider.js` è **una singola funzione hardcoded** (`listEntries`), importata direttamente da `syncService.js` — non esiste ancora un vero "seam" (un dispatch per tipo di sorgente, un registro di provider) da poter verificare o estendere. Da definire da zero se e quando si riprende: cosa deve fare concretamente un provider `channel` (rilevare nuovi upload di un canale intero), come si aggiunge un meccanismo di dispatch per tipo di sorgente senza rompere `playlistProvider.js` esistente, e cosa vuol dire "supportarlo" in modo verificabile (non solo una nota di sola lettura).
