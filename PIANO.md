...
3. ~~Rinominare "canali" in "creator"~~ — promosso a milestone **M15** (solo testo visibile; codice/schema/API restano `channel` — decisione esplicita dell'utente, non "da valutare" più).
4. **Verificare se il player `<video>` nativo basta** o serve un player più capace (controlli aggiuntivi, sottotitoli — già raccolti in `subtitleLanguagesAvailable` ma mai mostrati/caricati, velocità di riproduzione, ecc.). Da analizzare le reali funzionalità del player nativo del browser prima di decidere se serve una libreria player dedicata. **Picture-in-Picture già aggiunto in M16** — resta aperto il resto (sottotitoli, velocità, analisi generale).
...
9. **Analisi della pagina di riproduzione** (`VideoDetailPage`): quali funzioni deve davvero offrire il player, e ragionare sui "video suggeriti" (oggi mostra solo altri video dello stesso canale — vedi anche punto 4 sul player nativo). **Copertina "clicca per riprodurre" già aggiunta in M16, ma solo per i video `downloaded`** — resta aperto: mostrare la thumbnail come anteprima per gli stati **non ancora scaricati** (`new`/`pending`/`downloading`/`failed`/`excluded`, oggi lo sfondo vuoto a righe diagonali di `.player-placeholder`) — la thumbnail spesso non è ancora stata scaricata in quegli stati (solo `downloaded` la ha), da capire se recuperarla prima o mostrare quella remota; e i "video suggeriti".
