...
1. **Vista dello stato di un video oltre il locale** (scaricato / su cloud / solo su YouTube): oggi il catalogo traccia solo lo stato locale (`new/pending/downloading/downloaded/failed/excluded`). L'utente vuole poter vedere anche se un video esiste altrove (es. un backup cloud) oltre che localmente/su YouTube. Da definire: cosa si intende esattamente per "cloud" (quale servizio/provider), se è un campo informativo manuale o una verifica automatica, dove vive nello schema.
2. ~~Foto/avatar reali per i canali~~ — promosso a milestone **M14** (vedi tabella sopra).
3. **Rinominare "canali" in "creator"** in tutta l'interfaccia (CLI e web) e, da valutare, nel codice/schema (`channel` → `creator`?) — coerente con il linguaggio che l'utente usa per il progetto ("archivio canonico per creator" è già il nome usato per il layout cartelle).
4. **Verificare se il player `<video>` nativo basta** o serve un player più capace (controlli aggiuntivi, sottotitoli — già raccolti in `subtitleLanguagesAvailable` ma mai mostrati/caricati, velocità di riproduzione, ecc.). Da analizzare le reali funzionalità del player nativo del browser prima di decidere se serve una libreria player dedicata.
5. **Migliorare la grafica della pagina "Scarica video"** (`SingleDownloadPage`) — l'utente la trova poco curata rispetto al resto della direzione "Cinema"; da rivedere layout/stile una volta chiarito cosa non convince.
6. **Cambiare il font** — quello attuale (Space Grotesk per i titoli, da mockup 1b) è stato segnalato **poco leggibile** dall'utente; da rivalutare la scelta tipografica.
7. **Cambiare logo e nome del prodotto** — l'attuale "CINÉ." (nome/logo scelti insieme al mockup 1b) non convince più l'utente; da decidere un nuovo nome/logo.
8. **Filtro + ordinamento in Home e nelle pagine canale** — oggi `CatalogPage`/`ChannelPage` mostrano solo i chip di stato come filtro, nessun ordinamento (data aggiunta/download, durata, titolo, ecc.) né altri filtri (es. per canale dalla Home). Da definire quali criteri servono davvero.
9. **Analisi della pagina di riproduzione** (`VideoDetailPage`): quali funzioni deve davvero offrire il player, e ragionare sui "video suggeriti" (oggi mostra solo altri video dello stesso canale — vedi anche punto 4 sul player nativo). Inoltre: **mostrare la thumbnail come anteprima** al posto dello sfondo vuoto a righe diagonali quando il video non è ancora disponibile per la riproduzione (`new`/`pending`/`downloading`/`failed`/`excluded`) — la thumbnail spesso non è ancora stata scaricata in quegli stati (solo `downloaded` la ha), da capire se recuperarla prima o mostrare quella remota.
10. **Funzione di backup e ripristino** del catalogo (`data/catalog.json`, `data/metadata.json`, `data/config.json`) — e da valutare se includere anche i file in `media/`. Da definire: cosa copre esattamente, dove salva il backup (locale/esterno), se manuale (comando dedicato) o automatico, formato (semplice copia vs archivio).
11. **Ragionare se tenere lo storico job** (oggi `JobsPage`/`data/jobs/` accumulano senza limite ogni "Scarica in coda"/"Scarica video singolo" mai eseguito) e, se sì, **migliorarne la grafica** — da definire: serve davvero uno storico illimitato o va troncato/archiviato dopo un po', e come renderlo più leggibile di un semplice elenco (raggruppamento, ricerca, filtri per esito/tipo).
12. **Mostrare i dati tecnici del video** in `VideoDetailPage` — oggi il catalogo li ha già tutti (`video.container`/`videoCodec`/`audioCodec`/`bitrateKbps`/`sizeBytes`/`resolution`/`ytdlpVersion`/`sha256`, ecc., vedi schema) ma la pagina di dettaglio non ne mostra nessuno. Da definire: quali campi contano davvero per l'utente e dove metterli (sezione a comparsa? sempre visibile?).
