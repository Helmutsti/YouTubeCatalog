...
| M26 ⬜ | **Ingest a due fasi con metadati completi.** Nel `core`: `addSource`/`syncSource` in due fasi — flat-playlist istantaneo (video subito visibili) + nuovo job `enrichSource` che estrae i metadati completi per-video e cacha le copertine in locale, emettendo avanzamento (N di M) sull'`EventEmitter` del job manager. Adapter: in `SourcesPage` il pulsante **"Sync"** lancia il job e mostra una **barra di avanzamento che si riempie fino a fine sync** (riuso di `useJobStream`/SSE, nessun meccanismo nuovo). `presence: 'present'` sui video trovati. | Su una playlist reale: i video compaiono subito, poi il job arricchisce metadati+copertine con la barra di avanzamento che avanza fino al 100%; copertina conservata anche simulando un video poi rimosso. |
| M27 ⬜ | **Detection "Rimosso" nel refresh.** `syncSource` marca `presence: 'removed'` + `removedAt` i video non più trovati nella fonte, **reversibile** (riappaiono → `present`), **mai** cancellati (file/metadati intatti). | Su dati reali/sintetici: video rimosso da una playlist → marcato `removed`, file conservato; reintrodotto → torna `present`. |
| M28 ⬜ | **Pagina "Libreria".** Logica di filtro/ordinamento come funzioni pure nel `core`/`lib` condiviso; la pagina è un adapter. Vista dell'intera libreria: filtri per flag (presente/rimosso/scaricato/nascosto) + creator + sorgente, ordinamento (riuso `lib/sort.js`, M19), azioni rapide di cambio stato per-video + selezione multipla per download in blocco. "Novità" come filtro derivato; ritiro definitivo di "Rivedi novità"/coda. | Verifica reale nel browser sul catalogo dell'utente: ogni filtro/ordinamento corretto, cambio stato per-video e download multiplo funzionanti. |
| M29 ⬜ | **Aggiunta rapida in Libreria + riorganizzazione navigazione.** In testa alla pagina Libreria, il form "incolla un link" con un checkbox **"Download immediato"**: se spuntato scarica subito (comportamento attuale), se **non** spuntato aggiunge solo il video alla libreria senza scaricarlo (stub `presence:'present'`, `download:'none'`, nessun job). Core: `prepareSingleVideoDownload(input, { download })` — con `download:false` crea lo stub e ritorna senza lanciare job (poggia sul modello a flag di M25: un video può esistere "presente ma non scaricato"). La pagina "Scarica video" (`SingleDownloadPage`) diventa **"Cronologia"** (solo lo storico job, `JobHistory`), spostata **in basso** nel menu con **nuova icona**. | Verifica reale nel browser: checkbox off → il video compare in Libreria senza download; checkbox on → scarica; "Cronologia" mostra solo lo storico; menu riordinato con la nuova icona. |
| M30 ⬜ | **Chiedere "Vuoi tenere il video?" quando si nasconde un video scaricato.** Nascondere un video **scaricato** mostra la domanda **"Vuoi tenere il video?"**: se **Sì**, il file resta su disco (video scaricato ma nascosto); se **No**, viene cancellato **solo il file sul disco** e `download` torna a `'none'` — **il video resta comunque in libreria** (entry di catalogo, metadati e copertina **restano**, coerente con "i record non si perdono mai"). Nessun prompt se il video non è scaricato (niente da cancellare). Core: nuova funzione (es. `deleteVideoFile(id)`) + `setVideoHidden` invariato; il prompt vive negli adapter (CLI `confirm`, web modale). **Prima feature che cancella file media reali dal disco** → cautela `CLAUDE.md`: verifica con conteggio prima-dopo e pulizia dell'eventuale cartella creator rimasta vuota. | Verifica reale su dati di test: nascondere un video scaricato → domanda "Vuoi tenere il video?"; **No** rimuove il solo file (verificato su disco) e porta `download:'none'` mantenendo entry/metadati/copertina in libreria; **Sì** lascia il file; nessun prompt su un video non scaricato. |
...
> **Nota**: il vecchio punto 1 ("Vista dello stato di un video oltre il locale") è stato **promosso** a milestone M25–M28 — il modello a flag ortogonali (`presence`/`download`/`hidden`) lo copre. Rimosso da qui e gli altri punti rinumerati, come da regola del backlog. L'idea specifica di un "backup su cloud" resta assorbita dal punto 2 (backup e ripristino).
