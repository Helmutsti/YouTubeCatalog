...
7. **Anteprima video sulla card / nel player** (da ragionare) — una voce di menu ⋮ "Anteprima" che mostri un'anteprima del video: hover-preview sulla copertina in stile YouTube (uno spezzone che parte al passaggio del mouse) e/o un player di anteprima. Da definire: cosa si riproduce se il video non è scaricato (spezzone da YouTube? clip locale?), la sorgente dell'anteprima, dove vive (overlay sulla card vs mini-player), e il costo/complessità.
8. **Aggiunta istantanea nei Sorgenti** (da ragionare) — nella pagina Sorgenti, poter aggiungere subito una fonte/video in modo **istantaneo**, rimandando a un momento successivo sia il **download** sia la definizione delle **impostazioni**. Da definire (idea ancora acerba): cosa vuol dire esattamente "istantaneo" (cosa viene registrato subito), cosa viene rimandato e come/quando si completano download e impostazioni dopo, e come si presenta nell'interfaccia.
9. **Relazione video ↔ sorgenti** (da ragionare) — i video devono per forza essere collegati a una sorgente? Cosa succede ai video quando si **cancella** una sorgente, e cosa quando la si **riaggiunge**? Comportamento attuale (da cui partire): `removeSource` elimina solo la voce da `catalog.sources` lasciando intatti i video che la referenziano — il loro `source.sourceId` continua a puntare a una sorgente non più esistente (video "orfani"); riaggiungendo la stessa playlist la sorgente viene ricreata e le entry re-ingerite (dedup per id, così i video già presenti si "ricollegano" di fatto). Da definire il comportamento voluto: come gestire gli orfani (mostrarli comunque? etichettarli?), se la riaggiunta debba riconciliare esplicitamente, e se serva un concetto di video **svincolato** da qualsiasi sorgente (oggi esiste già per i download singoli, `source: { sourceId: null, type: 'single' }`).
