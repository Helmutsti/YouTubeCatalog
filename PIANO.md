...
9. **Titolo della scheda del browser dinamico** — cambiare `document.title` in base al contenuto: il titolo del video nella pagina di dettaglio, e il nome della pagina/sezione altrove (Home, Sorgenti, Archiviati, ecc.), invece del titolo statico "Ondo". Da definire: formato (es. `"<titolo video> · Ondo"`), dove impostarlo (hook per-pagina o effetto centralizzato sul router), e il ripristino al default.
10. **Anteprima video sulla card / nel player** (da ragionare) — una voce di menu ⋮ "Anteprima" che mostri un'anteprima del video: hover-preview sulla copertina in stile YouTube (uno spezzone che parte al passaggio del mouse) e/o un player di anteprima. Da definire: cosa si riproduce se il video non è scaricato (spezzone da YouTube? clip locale?), la sorgente dell'anteprima, dove vive (overlay sulla card vs mini-player), e il costo/complessità.
11. **Foto profilo del canale più grande** — ingrandire l'avatar del creator (sidebar e/o pagina creator/dettaglio). Micro-iterazione di stile quando la si affronta: bump delle dimensioni di `.avatar`/`.d-avatar`/`.chan-avatar` (o solo dove serve), verificando che il fallback iniziale-lettera resti centrato.
12. **Seconda barra di ricerca da rimuovere** — c'è una barra di ricerca ridondante (la searchbox nella topbar di `Layout` e quella dentro la pagina `Cerca`/`SearchPage`). Da rimuovere quella di troppo: probabilmente la searchbox interna a `SearchPage`, tenendo solo quella globale in topbar (che già naviga a `/search?q=`). Da confermare quale delle due tenere.
13. **Nuovo box in fondo al player video** (`VideoDetailPage`) — un nuovo riquadro sotto il player, da definire nei contenuti. Da chiarire cosa deve mostrare (es. capitoli, video correlati, commenti/note, dati/azioni aggiuntive?), la posizione esatta rispetto a titolo/descrizione/dati tecnici già presenti, e se dipende dallo stato del video (scaricato vs disponibile).
