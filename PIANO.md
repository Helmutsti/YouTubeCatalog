...
| M19 | **Filtro + ordinamento in Home e pagina canale**. Ordinamento (`CatalogPage`/`ChannelPage`, tutto client-side — i video sono già interamente caricati, nessuna modifica server): data aggiunta al catalogo (più recenti prima), data di pubblicazione originale su YouTube, durata, titolo (A-Z), per stato con i falliti in cima. Filtro: aggiunto anche un filtro per canale nella Home — oggi non possibile, `ChannelPage` esiste ma mostra solo i video già `downloaded` di quel canale, non permette di vedere ad es. i "nuovi" di un canale specifico senza guardare tutti i canali mescolati. | Da eseguire. |
| M20 | **Storico job: thumbnail del video + dettaglio errore**. Per `downloadSingle` (un solo video per job): a costo zero, `job.params.videoId` esiste già — basta recuperare e mostrare la thumbnail nello storico. Per `downloadPending` (più video in un job solo): richiede **modificare il job** per salvare l'elenco degli id scaricati/falliti nel `summary` (oggi solo `{downloaded, failed, total}`, nessun dettaglio per-video) — poi mostrare una fila di thumbnail (o la prima + "altri N") nello storico, decisione confermata con l'utente nonostante il costo maggiore. Dettaglio errore: `job.error.message` esiste già ma è visibile solo espandendo la riga — da promuovere alla vista collassata per i job falliti. | Da eseguire. |
| M21 | **Dati tecnici del video** in un box a fine descrizione in `VideoDetailPage`, stessa classe `.d-desc` già usata per il box "Descrizione" (come richiesto: "uguale alla descrizione"). Dati già tutti nel catalogo, mai mostrati: proposta di default risoluzione, codec video/audio, bitrate, dimensione file, container, versione yt-dlp (`sha256` escluso di default, poco interessante per un utente finale — da confermare se includerlo). | Da eseguire. |
