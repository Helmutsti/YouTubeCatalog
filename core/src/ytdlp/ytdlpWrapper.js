...
//
// M79 — li passiamo **tutti e quattro** invece del solo `node`. yt-dlp ne
// supporta quattro (`deno`, `node`, `quickjs`, `bun`) ma di default abilita solo
// `deno`; il flag è **additivo**, quindi elencarli tutti significa "usa il
// migliore che trovi". Inchiodarlo a `node` trasformava una dipendenza *di
// yt-dlp* in una dipendenza *inventata da noi*: su una macchina dove gira Deno
// e non Node — o dove Node non è nel PATH del processo — i download morivano con
// un 403 senza motivo apparente. Che *questo* progetto giri su Node non c'entra:
// il runtime qui serve a yt-dlp, che è un processo a sé.
...
async function finalizeDownload(paths, videoId) {
  ...
  // M78 — la risoluzione la dice il **file**, non i metadati. `info.width/height`
  // descrivono il miglior formato disponibile su YouTube: con un tetto di
  // risoluzione (M56) registravano 2160 per un file da 720. Qui si misura la
  // copia che abbiamo davvero. `probeResolution` non lancia mai: se ffprobe non
  // c'è o non risponde, si tengono i numeri dei metadati.
  const measured = await probeResolution(path.join(paths.videosDir, videoFile));
  if (measured) {
    fields.resolution = {
      ...fields.resolution,
      width: measured.width ?? fields.resolution.width,
      height: measured.height ?? fields.resolution.height,
      fps: measured.fps ?? fields.resolution.fps
    };
  }
