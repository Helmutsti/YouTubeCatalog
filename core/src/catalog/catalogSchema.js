...
export function createEmptyCatalog() {
  return {
    ...
    sources: {},
    channelAvatars: {},
    // I link incollati che NON hanno ancora un id (M85). Un link appena
    // incollato non è ancora un video: finché yt-dlp non l'ha risolto non ha id,
    // titolo né autore, quindi non è rappresentabile in `videos` — che è una
    // mappa per id. Un link che fallisce *prima* di risolversi resta qui col suo
    // motivo, invece di sparire senza lasciare traccia.
    // Forma: [{ url, addedAt, attempts, error, maxHeight }]
    queue: [],
