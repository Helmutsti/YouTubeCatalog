...
import { getVideo, listVideos, setHidden, setFavorite, deleteVideo } from '../api/client.js';
...
export function VideoDetailPage() {
  ...
  async function handleAction(kind, label) {
    try {
      if (kind === 'download') {
        await startDownload(id, { onSettled: reload, title: video.title });
        return;
      }
      if (kind === 'hide') {
        requestHide(video);
  ...
  async function handleRelatedAction(rid, kind) {
    ...
    try {
      if (kind === 'download') {
        await startDownload(rid, { onSettled: reload, title: target?.title });
        return;
      }
      if (kind === 'hide') {
        requestHide(target);
